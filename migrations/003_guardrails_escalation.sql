...
-- The turn record needs session_id, which was omitted earlier. The loop breaker
...
-- A refusal offers a callback pre-filled, so the member does not restate their
-- question. Nothing is sent anywhere: the form validates, stores and confirms.
create table if not exists callbacks (
  id                 uuid primary key default gen_random_uuid(),
  created_at         timestamptz not null default now(),
  -- Redacted before insert, same as turns.question.
  ...
  note               text,
  -- Deliberately no name, email or phone: this holds no member identity,
...
-- Per-session and per-IP limits. In Postgres rather than in
